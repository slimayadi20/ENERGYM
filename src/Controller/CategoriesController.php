...
class CategoriesController extends AbstractController
{
    ...
    /**
     * @Route("/ShowPoduit/{id}", name="ShowProduit")
     */
    public function ShowEvents(\Symfony\Component\HttpFoundation\Request $req, $id)
    {
        $em = $this->getDoctrine()->getManager();
        $categories = $em->getRepository(Categories::class)->find($id);

        //     var_dump($categorie);die();



        $listProduit = $em->getRepository(Produit::class)->findBy(['categories'=>$categories]);//list des prod ta3 categorie ly clikit alih




        return $this->render('shop/ShowProduit.html.twig', array('listProduit'=>$listProduit
